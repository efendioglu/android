...
public class PagerSlidingTabStrip extends HorizontalScrollView {
    ...
    public PagerSlidingTabStrip(Context context, AttributeSet attrs, int defStyle) {
        ...
        alignBottom = a.getBoolean(R.styleable.PagerSlidingTabStrip_pstsAlignBottom, alignBottom);
        a.recycle();
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (triangleIndicator) {
            ...
            int top = alignBottom ? 0 : (height - indicatorHeight);
            int right = (int) left + indicatorHeight + 30;
            int bottom = alignBottom ? indicatorHeight : height;
            r.set(left, top, right, bottom);
            Path path = getEquilateralTriangle(r);
            canvas.drawPath(path, rectPaint);
        } else {
            canvas.drawRect(lineLeft, alignBottom ? 0 : (height - indicatorHeight), lineRight, alignBottom ? indicatorHeight : height, rectPaint);
        }
//		canvas.drawRect(lineLeft, height - indicatorHeight, lineRight, height, rectPaint);
    ...
    public Path getEquilateralTriangle(Rect bounds) {
        ...
        startPoint = new Point(bounds.left, alignBottom ? bounds.top : bounds.bottom);

        p2 = new Point(startPoint.x + width, startPoint.y);
        //p3 = new Point(startPoint.x + (width / 2), startPoint.y - width);
        p3 = new Point(startPoint.x + (width / 2), alignBottom ? (startPoint.y + height) : (startPoint.y - height));
